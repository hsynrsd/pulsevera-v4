...
export default function PricingCard({ item, user }: {
    ...
    return (
        <Card className={`w-[350px] relative overflow-hidden ${item.popular ? 'border-primary shadow-xl scale-105' : ''}`}>
            {item.popular && (
                <div className="absolute inset-0 bg-primary/10 opacity-30" />
            )}
            <CardHeader className="relative">
                {item.popular && (
                    <div className="px-4 py-1.5 text-sm font-medium text-primary-foreground bg-primary rounded-full w-fit mb-4">
                        Most Popular
                    </div>
                )}
